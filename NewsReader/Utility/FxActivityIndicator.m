...
@implementation FxActivityIndicator
...
- (void)setMode:(FxActivityMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    //，
    if (mode == FxActivityModeDeterminate) {
        if ([NSThread isMainThread]) {
            [self updateProgress];
            [self setNeedsDisplay];
        } else {
...
- (void)updateProgress {
    [(FxRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == FxActivityModeDeterminate) {
        self.indicator = [[[FxRoundProgressView alloc] init] autorelease];
    }
    else if (mode == FxActivityModeCustomView && self.customView != nil){
...
+ (FxActivityIndicator *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    FxActivityIndicator *hud = [[FxActivityIndicator alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ...
    for (UIView *v in [view subviews]) {
        if ([v isKindOfClass:[FxActivityIndicator class]]) {
            viewToRemove = v;
        }
    }
    if (viewToRemove != nil) {
        FxActivityIndicator *HUD = (FxActivityIndicator *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
    ...
    if (!view) {
        [NSException raise:@"FxActivityViewIsNillException"
                    format:@"The view used in the FxActivity initializer is nil."];
    }
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = FxActivityAnimationFade;
        self.mode = FxActivityModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == FxActivityAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == FxActivityAnimationZoom) {
...
@implementation FxRoundProgressView
